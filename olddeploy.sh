...
if [ "$1" == "install" ]; then
    ...
    kubectl apply -f jenkins-ingress.yaml || { echo "Error: Failed to apply jenkins-ingress.yaml"; exit 1; }
    echo "Applying Grafana Ingress..."
    kubectl apply -f grafana-ingress.yaml || { echo "Error: Failed to apply grafana-ingress.yaml"; exit 1; }
